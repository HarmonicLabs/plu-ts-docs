...
:::info redeemer validator
For this smart contract we're using the `makeRedeemerValidator` method instead of the regular `makeValidator`.<br />
And this is because we don't need a Datum, we'll be minting tokens and storing them on the user wallet.
:::
