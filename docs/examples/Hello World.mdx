...
To get the protocol parameters we are using a `Blockfrost` object; this is exported from the `src/offchain/blockfrost.ts` file

```ts title="src/offchain/blockfrost.ts"
...
:::caution Add your Blockfrost Project ID

In order to use the Blockfrost API you need you need to log in to [blockfrost.io](https://blockfrost.io/), create your project and retrieve the API token.

:::

The file is really simple, and this is because we are using the [`blockfrost-pluts`](../tools/blockfrost-pluts) package.

Here we construct a `Blockfrost` instance that will do the Blockfrost REST API calls for us.
  ...
  if (typeof network === "number" && network !== 0) {
    return (
      <div className={[style.pageContainer, "center-child-flex-even"].join(" ")}>
        <b style={{margin: "auto 10vw"}}>
          Make sure to set your wallet in testnet mode;<br/>
          We are playing with founds here!
        </b>
        <Button onClick={() => window.location.reload()} style={{margin: "auto 10vw"}}>Refresh page</Button>
      </div>
    )
  }
