...
To submit the transaction we will use [`koios-pluts`](../tools/koios-pluts), a wrapper on top of the [koios](https://www.koios.rest/) API that uses `plu-ts` offchain types;
...
"dependencies": {
    ...
    "@harmoniclabs/koios-pluts": "^0.1.5",
...
export async function lockTx( wallet: BrowserWallet): Promise<string>
{
    ...
    const response = await fetch('/koios/tx/submit', {method: 'POST', body: JSON.stringify({txStr})});
...
import { BrowserWallet } from "@meshsdk/core";
import getTxBuilder from "./getTxBuilder";
import { Address, isData, DataB, Tx, UTxO } from "@harmoniclabs/plu-ts";
...
export default async function getTxBuilder(): Promise<TxBuilder>
{
    ...
    {
        const response = await fetch('/koios/epoch/protocol-params');
...
However (as the comment above explains) these parameters might not reflect reality so should be used with caution.

:::

To get the protocol parameters we are using a `koios` object; this is exported from the `src/offchain/koios.ts` file
