...
So in `src/contract.ts` we'll change the first `data` to [`PPubKeyHash`](../onchain/API/types/V1/PubKey/PPubKeyHash)
...
All the information about the transaction is in the `tx` field of the [`PScriptContext`](../onchain/API/types/V1/ScriptContext/PScriptContext.mdx)
