...
Here we are using the `peqInt` builtin function; the `$` method is a short form for the [`papp` function](./Function_application) and is how we pass arguments to a `plu-ts` function (we'll cover function application in the very next section).
